...
function listenMessages(id,type){
  ...
  const ref=database.ref(path).limitToLast(100),handler=s=>{box.innerHTML="";if(!s.exists()){box.innerHTML="<div class='muted'>Nenhuma mensagem ainda.</div>";return;}s.forEach(c=>{renderMessage(box,c.val());});box.scrollTop=box.scrollHeight;};
